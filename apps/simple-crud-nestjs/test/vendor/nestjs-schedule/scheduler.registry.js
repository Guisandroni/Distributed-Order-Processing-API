...
let SchedulerRegistry = SchedulerRegistry_1 = class SchedulerRegistry {
    ...
    getCronJob(name) {
        ...
        if (!ref) {
            throw new Error((0, schedule_messages_js_1.NO_SCHEDULER_FOUND)('Cron Job', name));
        }
        return ref;
    }
    getInterval(name) {
        ...
        if (typeof ref === 'undefined') {
            throw new Error((0, schedule_messages_js_1.NO_SCHEDULER_FOUND)('Interval', name));
        }
        return ref;
    }
    getTimeout(name) {
        ...
        if (typeof ref === 'undefined') {
            throw new Error((0, schedule_messages_js_1.NO_SCHEDULER_FOUND)('Timeout', name));
        }
        return ref;
    }
    addCronJob(name, job) {
